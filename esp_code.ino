...
bool httpRequest(
  ...
  if (method == "GET") {
    ...
    uint8_t* data = const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(payload.c_str()));
    statusCode = http.sendRequest("PATCH", data, payload.length());
...
bool firebaseSignIn() {
  ...
  reqDoc["email"] = FIREBASE_EMAIL;
  reqDoc["password"] = FIREBASE_PASSWORD;
  ...
  StaticJsonDocument<1536> resDoc;
  ...
  gIdToken = String(resDoc["idToken"] | "");
  gUserUid = String(resDoc["localId"] | "");
  int expiresInSec = String(resDoc["expiresIn"] | "3600").toInt();
...
void publishDispenseResult(const DispenseCommand& cmd, bool ok, const String& message) {
  ...
  doc["online"] = true;
  doc["lastSeen"] = epochMs();
  doc["last_dispense"]["slot"] = cmd.slot;
  doc["last_dispense"]["medication"] = cmd.medication;
  doc["last_dispense"]["commandTimestamp"] = cmd.timestamp;
  doc["last_dispense"]["processedAt"] = epochMs();
  doc["last_dispense"]["status"] = ok ? "ok" : "error";
  doc["last_dispense"]["message"] = message;
...
CommandFetchResult fetchDispenseCommand(DispenseCommand& cmdOut) {
  ...
  if (err) {
    Serial.printf("[DB] Command parse error: %s\n", err.c_str());
    return COMMAND_ERROR;
  }

  cmdOut.slot = doc["slot"] | 0;
  cmdOut.medication = String(doc["medication"] | "");
  cmdOut.timestamp = doc["timestamp"] | 0ULL;
  cmdOut.action = String(doc["action"] | "");
...
void processDispenseCommand() {
  ...
  prefs.putULong64("last_cmd_ts", gLastHandledTimestamp);
...
void setup() {
  ...
  gLastHandledTimestamp = prefs.getULong64("last_cmd_ts", 0);
