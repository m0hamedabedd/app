...
export const Reports: React.FC<ReportsProps> = ({ logs, medications }) => {
    ...
    <div className="space-y-6 pb-24 animate-fade-in">
      <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-800 flex items-center">
            <i className="fas fa-chart-line text-teal-600 mr-2"></i>
            Analytics
          </h2>
      ...
      <div className="bg-gradient-to-r from-violet-600 to-indigo-600 rounded-2xl p-5 shadow-lg text-white">
        <div className="flex justify-between items-start mb-3">
             <div className="flex items-center space-x-2">
                ...
                <h3 className="font-bold">AI Health Insights</h3>
             </div>
             {!aiInsights && (
                <button 
                    onClick={handleGenerateInsights}
                    disabled={aiLoading}
                    className="bg-white/20 hover:bg-white/30 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition-all backdrop-blur-sm flex items-center space-x-1"
                >
        ...
        {aiLoading ? (
            ...
            <p className="text-sm text-indigo-100 opacity-80">
                Tap generate to get personalized insights about your medication adherence patterns and habits.
            </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
          {/* Adherence Card */}
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col justify-between relative overflow-hidden h-36">
             <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-br from-gray-50 to-white rounded-bl-full z-0"></div>
             <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider z-10">Adherence</h3>
             <div className="flex items-center mt-2 z-10 flex-1">
                 ...
                 <div className="flex flex-col justify-center">
                     ...
                     <p className="text-[10px] text-gray-400 mt-1 leading-tight">Completion Rate</p>
                 </div>
             </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-teal-500 to-emerald-600 rounded-2xl p-5 shadow-lg text-white flex flex-col justify-between relative overflow-hidden h-36">
              ...
              <h3 className="text-teal-100 text-xs font-bold uppercase tracking-wider">Streak</h3>
              <div className="flex-1 flex flex-col justify-center">
                  <div className="flex items-baseline">
                      <span className="text-4xl font-bold">{stats.currentStreak}</span>
                      <span className="text-sm font-medium ml-1 text-teal-200">days</span>
                  </div>
                  <p className="text-[10px] text-teal-200 mt-1">Consecutive perfect days</p>
              </div>
          </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
          <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 text-center">
              ...
              <p className="text-[10px] text-gray-400 uppercase font-semibold">Taken</p>
          </div>
          <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 text-center">
              <p className="text-red-500 font-bold text-xl">{stats.missed}</p>
              <p className="text-[10px] text-gray-400 uppercase font-semibold">Missed</p>
          </div>
          <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 text-center">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-3 text-xs">
                  <div className="flex items-center"><span className="w-2 h-2 rounded-full bg-teal-500 mr-1"></span> Taken</div>
                  <div className="flex items-center"><span className="w-2 h-2 rounded-full bg-red-400 mr-1"></span> Missed</div>
              </div>
          </div>
          
          <div className="flex items-end justify-between h-40 space-x-1 sm:space-x-3">
                  ...
                  return (
                      <div key={idx} className="flex flex-col items-center flex-1 h-full justify-end group cursor-pointer">
                          ...
                          <div className="absolute mb-8 hidden group-hover:block bg-gray-800 text-white text-[10px] p-2 rounded shadow-lg z-20 pointer-events-none whitespace-nowrap">
                             {day.date}: {day.taken} taken, {day.missed} missed
                          </div>
                      </div>
                  );
              })}
      ...
      {/* Medication Breakdown */}
      <div>
          <h3 className="font-bold text-gray-800 mb-4 px-1">Medication Performance</h3>
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-50">
              {stats.medStats.length === 0 ? (
                  ...
                  stats.medStats.map(med => (
                      <div key={med.id} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                          <div className="flex items-center space-x-3">
                              ...
                              <div>
                                  ...
                                  <p className="text-xs text-gray-400">{med.stats.total} scheduled doses</p>
                              </div>
                          </div>
